...
# Example:
data modify storage pk:common package.search_block.args set value {distance:5,block:"#pk_pa_ta:player_heads",nbt:"",callback:"setblock ~ ~ ~ stone",stop_at_first:true}
function pk_pa_ta:packages/search_block/start with storage pk:common package.search_block.args
# Would search a block matching the tag #pk_pa_ta:player_heads in a 10x10x10 area where the current location is the center
