...
execute if score $pk.pa_ta.settings.tick_clear_gui_items pk.value matches 1 run tellraw @s [{text:"Setting \"",color:"gray"},{text:"Tick Clear GUI Items",color:"yellow"},{text:"\" has been set to ",color:"gray"},{text:"false",color:"yellow"}]

# Update the setting's value
scoreboard players set $pk.pa_ta.settings.tick_clear_gui_items pk.value 0